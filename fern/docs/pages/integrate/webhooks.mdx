...
| Aspect | What Talkif sends |
|---|---|
| Method, URL, body | Exactly as declared on the flow function, with the model's arguments merged in |
| Headers | Your function's `webhookHeaders` (e.g. `Authorization`), `Content-Type: application/json` when there's a body, and `Talkif-Signature` once a secret exists |
| Timeout | The function's `timeoutMs` (100 ms – 30 s) |
| Body size | Requests ≤ 1 MB; the first 1 MB of your response is read |
| Redirects | Not followed — respond directly with a 2xx/3xx |
| Reachability | Public internet only; private and cloud-metadata addresses are refused |
| Source | Fixed egress addresses per region — see [Network and IP allowlists](/reference/network-and-ip-allowlists) |
